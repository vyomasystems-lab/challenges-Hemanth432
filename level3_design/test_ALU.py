    ...
    for a in range(1):
        for b in range(16):
            for ci in range(2):
                    ...
                    dut._log.info(f'a={a:04} b={b:04} ci={ci:01} M=1 s=0  OUT={int(dut.y.value):04}')
...
async def test_alu_a1(dut):
        for b in range(16):
            for ci in range(2):
                    ...
                    dut.s.value = 0            
                    await Timer(2, units='ns')
                    dut._log.info(f'a=1 b={b:04} ci={ci:01} M=1 s=0  OUT={int(dut.y.value):04}')
...
async def test_alu_a2(dut):
        for b in range(16):
            for ci in range(2):
                    ...
                    dut.s.value = 0            
                    await Timer(2, units='ns')
                    dut._log.info(f'a=2 b={b:04} ci={ci:01} M=1 s=0  OUT={int(dut.y.value):04}')
...
async def test_alu_a3(dut):
        for b in range(16):
            for ci in range(2):
                    ...
                    dut.s.value = 0            
                    await Timer(2, units='ns')
                    dut._log.info(f'a=3 b={b:04} ci={ci:01} M=1 s=0  OUT={int(dut.y.value):04}')
        ...
        for a in range(16):
            for b in range(16):
                for ci in range(2):
                    ...
                    dut.s.value = 3         
                    await Timer(2, units='ns')
                    dut._log.info(f'a={a:04} b={b:04} ci={ci:01} M=1 s=3 OUT={int(dut.y.value):04}')
